...
public class ParsedRecurringEventTest {
  ...
  public void testDefaultConstructor() {
    ...
    assertEquals("Weekdays should be empty string by default", "", recurringEvent.getWeekdays());
    assertEquals("Repeat count should be 0 by default", 0, recurringEvent.getRepeatCount());
    assertNull("RepeatUntil should be null by default", recurringEvent.getRepeatUntil());
  ...
  public void testSetAndGetIsRecurring() {
    ...
    assertTrue("isRecurring should be true when set to true", recurringEvent.isRecurring());

    recurringEvent.setRecurring(false);
    assertFalse("isRecurring should be false when set to false", recurringEvent.isRecurring());
  }

  @Test
  public void testSetAndGetWeekdays() {
    recurringEvent.setWeekdays("MTWRF");
    assertEquals("Weekdays should be set correctly", "MTWRF", recurringEvent.getWeekdays());

    // Edge case: Empty string
    recurringEvent.setWeekdays("");
    assertEquals("Weekdays should be empty when set to empty", "", recurringEvent.getWeekdays());
  }

  @Test
  public void testSetAndGetRepeatCount() {
    recurringEvent.setRepeatCount(5);
    assertEquals("Repeat count should be set correctly", 5, recurringEvent.getRepeatCount());

    // Edge case: Negative count
    recurringEvent.setRepeatCount(-1);
    assertEquals("Repeat count should be -1 when set to -1", -1, recurringEvent.getRepeatCount());

    // Edge case: Zero count
    recurringEvent.setRepeatCount(0);
    assertEquals("Repeat count should be 0 when set to 0", 0, recurringEvent.getRepeatCount());
  }
  ...
  public void testSetAndGetRepeatUntil() {
    ...
    assertEquals("RepeatUntil should be set correctly", repeatUntilDate, recurringEvent.getRepeatUntil());

    // Edge case: Setting to null
    recurringEvent.setRepeatUntil(null);
    assertNull("RepeatUntil should be null after setting null", recurringEvent.getRepeatUntil());
  }
